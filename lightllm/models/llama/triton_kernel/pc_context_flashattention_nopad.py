...
if triton.__version__ >= "2.1.0":
    ...
    def _fwd_kernel(
        ...
        cache_size = tl.load(b_cache_size + cur_batch)
        ...
        block_end_loc = tl.minimum((start_m + 1) * BLOCK_M + cache_size, cur_batch_seq_len + cache_size)

        for start_n in range(0, block_mask * block_end_loc , BLOCK_N):
            start_n = tl.multiple_of(start_n, BLOCK_N)
            # -- compute qk ----
            kv_loc = tl.load(Req_to_tokens + stride_req_to_tokens_b * cur_batch_req_idx + stride_req_to_tokens_s * (start_n + offs_n),
            ...
            qk += tl.dot(q, k)
            qk *= sm_scale
            qk = tl.where(offs_m[:, None] + cache_size >= start_n + offs_n[None, :] and start_n + offs_n[None, :] < block_end_loc, qk, float("-inf"))
            ...
            acc_scale = tl.where(offs_m + cache_size >= start_n, acc_scale, 1.0)
            ...
            p = tl.where(offs_m[:, None] + cache_size >= start_n + offs_n[None, :], p, 0.0)
            p = p.to(v.dtype)
            acc += tl.dot(p, v)
            # update m_i and l_i
            l_i = l_i_new
    ...
    @torch.no_grad()
    def pc_context_attention_fwd(q, k, v, o, b_req_idx, b_start_loc, b_seq_len, b_cache_size, max_input_len, req_to_token_indexs):
        BLOCK = 128
        BLOCK = 128
        # shape constraints
        ...
        _fwd_kernel[grid](
            ...
            o, req_to_token_indexs, b_req_idx,
...
def torch_att(xq, xk, xv, bs, seqlen, num_head, head_dim, cache_size):
    xq = xq.view(bs, seqlen, num_head, head_dim)
    xk = xk.view(bs, seqlen + cache_size, num_head, head_dim)
    xv = xv.view(bs, seqlen + cache_size, num_head, head_dim)
    mask_cache = torch.ones((seqlen, cache_size)).cuda().unsqueeze(0).unsqueeze(0).cuda()
    mask = torch.tril(torch.ones(seqlen, seqlen), diagonal=0).unsqueeze(0).unsqueeze(0).cuda()
    mask[mask == 0.] = -100000000.0
    print(mask.shape, mask_cache.shape)
...
def test():
    ...
    b_cache_size = torch.zeros(1, dtype=torch.int32, device='cuda')
    b_cache_size[0] = 524
    cache_size = 524
    ...
    # b_seq_len[3] = 1024
    b_req_idx[0] = 0
    req_to_token_indexs[0][:cache_size + N_CTX] = torch.tensor(np.arange(cache_size + N_CTX), dtype=torch.int32).cuda()
    ...
    for i in range(Z):
        ...
        torch_o = torch_att(q[start:end], k[start:end + cache_size], v[start:end + cache_size], 1, b_seq_len[i], H, D_HEAD, cache_size)
        start = end
    ...
    for i in range(10000):
        pc_context_attention_fwd(q, k, v, o, b_req_idx, b_start_loc, b_seq_len, b_cache_size, max_input_len, req_to_token_indexs)
    torch.cuda.synchronize()
    b = time.time()
